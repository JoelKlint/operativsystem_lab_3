...
// Make sure page to be removed is saved safely to disc.
// Return physical page index that can be overwritten
static unsigned take_phys_page()
{
	unsigned		page;	/* Page to be replaced. */

	// Physical page that should be saved to disc
	page = (*replace)();
	// Page table to written to disc
	page_table_entry_t* page_entry = coremap[page].owner;

	if(page_entry != NULL) {
		// Write to which swap page?
		...
		page_entry->page = swap_page; // Page can now be found in swap file
		//page_entry->ondisk = 1; // Should this be set here?
		page_entry->inmemory = 0;

	}
	// Return the physical page that can be overwritten
	return page;
}

// Load the virtual page into memory
static void pagefault(unsigned virt_page)
{
	...
	// Return the page from memory that should be replaced
	page = take_phys_page();

	// Make the actual change
	page_table_entry_t* page_entry = &page_table[virt_page];
	
	unsigned swap_page;
	if (page_entry->ondisk) {
		swap_page = page_entry->page;
		read_page(page, swap_page);
	} else {
		swap_page = new_swap_page();
	}

	page_entry->page = page;
	page_entry->inmemory = 1;
	//page_entry->ondisk = 0; // Should this be set?
	page_entry->modified = 0;
	page_entry->referenced = 0;

	coremap[page].owner = page_entry;
